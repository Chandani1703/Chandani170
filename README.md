# Chandani1703

# Hi, I'm Chandani Patel 👋

🎓 Computer Science student (CS4A) at Leeds Beckett University 
👨‍🏫 Tutors: Tony Jenkins, Patrick Ingham  
🔭 Working on Python projects and practical coding exercises  
🌱 Interested in: Automation, Web Development, Data Science  
💡 Always learning: Best practices in Python, Git, and software design

## 🌟 Featured Projects
- **Beginner Python Programs**: Sorting numbers, user input/output 
- **Invoice Printer**: Exploring data structures in Python 
- **Dropshipping Research**: Notes and resources for starting in the UK
- **https://github.com/lbu-focp/programming-and-notes-Chandani1703.git

## 🛠️ Skills & Tools
Python • Git • PyCharm • VS Code • HTML/CSS • Jupyter • Research

## 📫 Connect with me
- [Email Address = chandnipatel00758@gmail.com]

![GitHub stats](https://github-readme-stats.vercel.app/api?username=your-github-username&show_icons=true&hide_title=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=your-github-username&layout=compact)

---

*“Code is like humor. When you have to explain it, it’s bad.”* — Cory House

---

## 📸 GitHub Profile

![GitHub Profile Screenshot](Profile%20Screenshot.png)
